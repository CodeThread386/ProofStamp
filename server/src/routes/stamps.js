...
async function generateCertificatePdf(stamp, passport) {
  ...
  if (stamp.expertSignature) {
    y -= 50;
    page.drawText('NOTIFIED EXPERT SIGNATURE (BSA PART B)', { x: 50, y, size: 10, font: fontBold, color: rgb(0.1, 0.5, 0.1) });
    y -= 16;
    page.drawText(`Advocate/Expert Name: ${stamp.expertName || 'N/A'}`, { x: 50, y, size: 9, font });
    y -= 14;
    page.drawText(`Registration/ID: ${stamp.expertId || 'N/A'}`, { x: 50, y, size: 9, font });
    y -= 14;
    page.drawText(`Signature: ${stamp.expertSignature}`, { x: 50, y, size: 9, font });
    y -= 14;
    page.drawText(`Date: ${stamp.expertSignatureDate ? new Date(stamp.expertSignatureDate).toISOString() : 'N/A'}`, { x: 50, y, size: 9, font });
    y -= 26;
  } else {
    y -= 120;
  }
  ...
  page.drawText('LEGAL DISCLAIMER:', { x: 50, y: 60, size: 8, font: fontBold, color: rgb(0.5, 0.5, 0.5) });
  page.drawText('ProofStamp creates evidentiary records. It is not proof of legal ownership and does not substitute', { x: 50, y: 50, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
  page.drawText('for copyright registration or legal counsel.', { x: 50, y: 40, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
